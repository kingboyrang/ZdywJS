...
- (void)InitMainView{
    ...
    CGFloat h=[[UIScreen mainScreen] bounds].size.height-64- _dialPlateView.frame.size.height;
    UIView *bgView=[[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, h)];
    bgView.tag=AdScrollTag;
...
- (void)dialUnflod{
    [UIView animateWithDuration:0.35 animations:^{
        ...
        [_dialPlateView setHidden:NO];
        if (finished) {//显示键盘
            if (!_dialPlateView.hasNumber&&![self.subviews containsObject:self.AdView]) {
    ...
    switch (dialPlateType) {
        ...
        case DialPlateType_DialFlod:{
            [UIView animateWithDuration:0.35 animations:^{
                ...
                [_dialPlateView setHidden:YES];
                if (finished) {
                    if (_dialPlateView.hidden) {//如果键盘隐藏
                        if ([self.subviews containsObject:self.AdView]) {
                            [self.AdView removeFromSuperview];
                        }
                    }else{
