...
- (void)receivePayCardResult:(NSNotification *)notification
{
    ...
    if ([result integerValue] == 0) // 注册成功
    {
        ...
        NSString * alertMsg = [NSString stringWithFormat:@"为了保证您的正常拨打，在您前三次拨打后,系统将提示您验证手机号.现在就去使用品鉴酒电话吧!"];
...
- (void)receivePhoneRegResult:(NSNotification *)notification
{
    ...
    if ([result integerValue] == 0) //
    {
        ...
        [SVProgressHUD dismissWithSuccess:@"欢迎使用品鉴酒!" afterDelay:2];
...
//    UIAlertView *AlertView = [[UIAlertView alloc] initWithTitle:@"品鉴酒提示"
//                                                        message:alertMsg
//                                                       delegate:self
